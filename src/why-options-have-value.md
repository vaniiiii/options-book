...
\\[ \Delta \text{Option Price} \approx (\text{stock sensitivity} \times \Delta \text{Stock}) + (\text{time sensitivity} \times \Delta \text{Time}) + (\text{volatility sensitivity} \times \Delta \text{IV}) + (\text{rate sensitivity} \times \Delta \text{Rates}) \\]
